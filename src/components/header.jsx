  ...
  return (
    <>
      ...
      <div className="font-jakarta text-slate-200">
        ...
        {}
        {/* Fixed wrapper strictly for centering, allowing the inner header to animate smoothly */}
        <div className="fixed top-0 inset-x-0 z-50 flex justify-center pointer-events-none px-4 sm:px-6">
          
          {}
          {/* 
            LAG FIX: Using translate-y instead of top-4, and cleanly transitioning max-w and rounded styles. 
            All animations are eased out gracefully.
          */}
          <header
            className={`pointer-events-auto flex items-center justify-between w-full transition-all duration-500 ease-out will-change-[max-width,transform,border-radius,background-color] ${
              isScrolled
                ? "max-w-5xl translate-y-4 rounded-full bg-slate-950/80 backdrop-blur-2xl shadow-[0_20px_40px_-10px_rgba(0,0,0,0.8)] py-3 px-5 sm:px-8"
                : "max-w-7xl translate-y-0 rounded-none bg-transparent shadow-none py-5 px-0"
            }`}
          >
            
            {}
            <a href="/" className="flex items-center gap-3 group outline-none z-50 relative shrink-0">
              <div className="relative flex items-center justify-center w-11 h-11 rounded-xl">
                <div className="absolute inset-0 bg-blue-500/20 rounded-xl blur-md opacity-40 transition-all duration-300 ease-out group-hover:opacity-100 group-hover:scale-110" />
                <div className="absolute inset-0 bg-[#0a0f1c] rounded-xl flex items-center justify-center transition-all duration-300 z-10 overflow-hidden shadow-[inset_0_0_15px_rgba(255,255,255,0.05)]">
                  ...
                  <Code2
                    className="w-5 h-5 text-blue-500 transition-transform duration-300 ease-out group-hover:scale-110 relative z-20"
                    strokeWidth={2.5}
                  />
                </div>
              </div>
              <div className="flex flex-col justify-center">
                <h2 className="text-2xl font-extrabold tracking-tight text-white leading-none m-0">
                  CodeLume<span className="text-blue-500 animate-pulse">.</span>
                </h2>
              </div>
            </a>
              ...
              >
                <button className="flex items-center gap-1.5 px-4 py-2 rounded-full outline-none hover:bg-white/5 transition-colors duration-300">
                  <h2 className="m-0 text-[14px] font-bold text-slate-300 group-hover/nav:text-white transition-colors flex items-center gap-1.5">
                    Services
                    <ChevronDown
                      className={`w-3.5 h-3.5 transition-transform duration-300 ease-out text-slate-500 group-hover/nav:text-blue-400 ${
                ...
                {}
                <div
                  className={`absolute top-[calc(100%+1rem)] left-1/2 -translate-x-1/2 w-[680px] pt-0 transition-all duration-300 ease-out origin-top ${
                    servicesOpen
                      ? "opacity-100 translate-y-0 visible scale-100"
                      : "opacity-0 -translate-y-3 invisible scale-95"
                  }`}
                >
                  <div className="bg-[#050b14]/95 backdrop-blur-3xl rounded-3xl shadow-[0_50px_100px_-20px_rgba(0,0,0,1)] p-4 relative overflow-hidden">
                    ...
                    <div className="absolute -top-32 right-0 w-64 h-64 bg-indigo-600/15 rounded-full blur-[80px] pointer-events-none" />
                        ...
                        return (
                          ...
                          >
                            <div className={`relative z-10 shrink-0 w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 ${service.bgColor} ${service.hoverBg}`}>
                              <Icon className={`w-5 h-5 ${service.color} transition-transform duration-300 ease-out group-hover:scale-110`} />
                            </div>
                            ...
                            <div className="relative z-10 flex flex-col pt-0.5">
                              <h2 className="text-[15px] font-extrabold text-slate-200 group-hover:text-white transition-colors flex items-center gap-1.5 m-0 leading-tight">
                                {service.title}
                                <ArrowRight className="w-3.5 h-3.5 opacity-0 -translate-x-2 transition-all duration-300 ease-out group-hover:opacity-100 group-hover:translate-x-0 text-blue-400" />
                              </h2>
                    ...
                    <div className="mt-4 bg-black/40 rounded-2xl p-5 flex items-center justify-between relative overflow-hidden">
                      ...
                      <a
                        href="#contact"
                        className="text-[14px] font-bold text-blue-400 hover:text-blue-300 flex items-center gap-1.5 group/link transition-colors relative z-10"
                      >
                        <h2 className="m-0 text-inherit text-[14px] font-bold flex items-center gap-1.5">
                          Consult with us <ArrowRight className="w-4 h-4 transition-transform group-hover/link:translate-x-1" />
                        </h2>
                      </a>
                    </div>
                  </div>
            ...
            <div className="flex items-center gap-4 relative z-50 shrink-0">
              <a
                href="#start-project"
                className="hidden md:flex relative group items-center justify-center gap-2 px-7 py-3 rounded-full bg-[#0F172A] text-white overflow-hidden outline-none shadow-[0_0_20px_rgba(37,99,235,0.2)] hover:shadow-[0_0_30px_rgba(37,99,235,0.4)] transition-all duration-300 hover:-translate-y-0.5"
              >
        ...
        {}
        <div
          className={`lg:hidden fixed inset-0 z-40 bg-[#030712]/95 backdrop-blur-3xl transition-all duration-500 ease-out flex flex-col justify-center ${
        ...
        >
          <div className="px-6 py-8 h-full flex flex-col pt-32 overflow-y-auto">
                ...
                {servicesData.map((service, idx) => {
                  ...
                  return (
                    <a
                      key={service.title}
                      href={`#${service.title.toLowerCase().replace(/ /g, "-").replace(/&/g, "").replace(/\//g, "-")}`}
                      className="flex items-center gap-5 p-4 rounded-2xl bg-white/[0.02] hover:bg-white/[0.05] transition-all duration-300 active:scale-[0.98] outline-none"
            ...
            <div className="space-y-2 mb-10 px-2">
              {standardLinks.map((item, idx) => (
                <a
                  key={item}
                  href={`#${item.toLowerCase().replace(/ /g, "-")}`}
                  className="block py-2 outline-none group"
                  onClick={() => setMobileMenuOpen(false)}
                  style={{
            ...
            >
              <a
                href="#start-project"
                className="relative flex items-center justify-center gap-2 w-full px-6 py-5 rounded-2xl bg-linear-to-r from-blue-600 to-indigo-600 text-white shadow-[0_15px_30px_rgba(37,99,235,0.3)] transition-all active:scale-[0.98] overflow-hidden outline-none"
              >
                <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent animate-shimmer pointer-events-none" />
