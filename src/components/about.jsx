...
import { UserCircle, ArrowRight, Palette, Terminal, TrendingUp } from "lucide-react";
...
export default function About() {
  const skills = [
    ...
    },
    {
      ...
      glow: "bg-cyan-500/10",
      alignment: "mr-auto lg:ml-6",
    }
  ];

  return (
    <section className="relative w-full py-20 lg:py-32 bg-[#030712] font-jakarta overflow-hidden" id="about">
      <style>
        {`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&display=swap');
          .font-jakarta { font-family: 'Plus Jakarta Sans', sans-serif; }`}
      </style>

      {/* 🚀 FIXED: Replaced expensive 'blur-[150px]' with GPU-friendly radial gradients */}
      <div className="absolute top-0 left-0 w-150 h-150 bg-[radial-gradient(circle_at_center,var(--tw-gradient-stops))] from-blue-900/15 via-blue-900/5 to-transparent pointer-events-none transform-gpu translate-z-0" />
      <div className="absolute bottom-0 right-0 w-125 h-125 bg-[radial-gradient(circle_at_center,var(--tw-gradient-stops))] from-indigo-900/15 via-indigo-900/5 to-transparent pointer-events-none transform-gpu translate-z-0" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-8 items-center">
          ...
          <div className="flex flex-col items-start text-left">
            ...
            <h2 className="text-base sm:text-lg text-slate-400 font-medium leading-relaxed max-w-lg mb-10 m-0">
              As the driving force behind CodeLume, I bridge the gap between stunning aesthetic design and robust server architectures. I don't rely on pre-built templates. Every pixel is optimised, every line of code is customised, and every platform is engineered for global scale.
            </h2>
          ...
          <div className="relative w-full flex flex-col gap-5 sm:gap-6 pt-8 lg:pt-0">
            ...
            {skills.map((skill, index) => {
              ...
              return (
                <div 
                  key={index}
                  // 🚀 FIXED: Reduced backdrop-blur-2xl to backdrop-blur-xl and softened shadows for Android GPU
                  className={`relative z-10 w-full sm:w-[85%] rounded-3xl bg-[#0a0f1c]/75 backdrop-blur-xl p-6 sm:p-8 shadow-[0_10px_30px_rgba(0,0,0,0.5),inset_0_0_15px_rgba(255,255,255,0.03)] hover:shadow-[0_15px_40px_rgba(0,0,0,0.6),inset_0_0_20px_rgba(255,255,255,0.05)] hover:-translate-y-1 transition-all duration-500 ease-out transform-gpu flex items-center gap-5 group outline-none ${skill.alignment}`}
                >
                  ...
                  <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl ${skill.glow} flex items-center justify-center shrink-0 shadow-[inset_0_0_15px_rgba(255,255,255,0.05)] group-hover:scale-110 transition-transform duration-500 transform-gpu relative z-10`}>
